...
pygame.font.init()
font = pygame.font.SysFont('simsun', 50)  # 字体 & 字号
...
for i in range(0,trial_num):
    ...
    if noask_result[i] == 0 :
        ...
        ns.sync()
        ns.send_event('0001', label='noask')
        # if noask :
        #     code = sendcode('66')
        ...
        #     code = sendcode('88')
        ns.sync()
        ns.send_event('0002', label='canask')
        time.sleep(1.5)
        ...
        pygame.display.update()
        ns.sync()
        ns.send_event('0024', label='choose1')
        time.sleep(1.5)
        win.fill((0, 0, 0))
        win.blit(ifask, (x_center - ifask_size[2]//2, y_center - ifask_size[3]//2))
    ...
    if context and if_ask :
        ns.sync()
        ns.send_event('0003', label='case1')
    elif (not if_ask) :
        ns.sync()
        ns.send_event('0004', label='donotask')
    else :
        ns.sync()
        ns.send_event('0005', label='case2')
    ...
    pygame.display.update()
    ns.sync()
    ns.send_event('0025', label='choose2')
    time.sleep(1.5)
    ...
    if context and if_ask :
        if apple[i] == 0 :
            ns.sync()
            ns.send_event('0006',  label='case1+0')
        elif apple[i] == 3 :
            ns.sync()
            ns.send_event('0007', label='case1+3')
        elif apple[i] == 6 :
            if resp_2[i] == 0:  #left
                ns.sync()
                ns.send_event('0008', label='Lcase1+6')
            else : #right
                ns.sync()
                ns.send_event('0009', label='Rcase1+6')
        elif apple[i] == 9 :
            ns.sync()
            ns.send_event('0010', label='case1+9')
        else :
            ns.sync()
            ns.send_event('0011', label='case1+12')
    elif (not if_ask) :
        if apple[i] == 0 :
            ns.sync()
            ns.send_event('0012', label='no+0')
        elif apple[i] == 3 :
            ns.sync()
            ns.send_event('0013',label='no+3')
        elif apple[i] == 6 :
            if resp_2[i] == 0:  #left
                ns.sync()
                ns.send_event('0014', label='Lno+6')
            else : #right
                ns.sync()
                ns.send_event('0015', label='Rno+6')
        elif apple[i] == 9 :
            ns.sync()
            ns.send_event('0016', label='no+9')
        else :
            ns.sync()
            ns.send_event('0017', label='no+12')
    else :
        if apple[i] == 0 :
            ns.sync()
            ns.send_event('0018', label='case2+0')
        elif apple[i] == 3 :
            ns.sync()
            ns.send_event('0019', label='case2+3')
        elif apple[i] == 6 :
            if resp_2[i] == 0:  #left
                ns.sync()
                ns.send_event('0020', label='Lcase2+6')
            else : #right
                ns.sync()
                ns.send_event('0021', label='Rcase2+6')
        elif apple[i] == 9 :
            ns.sync()
            ns.send_event('0022', label='case2+9')
        else :
            ns.sync()
            ns.send_event('0023', label='case2+12')
...
print('aaa')
date = (time.strftime(u"%Y_%b_%d_%H%M%S"))  # 获取时间
